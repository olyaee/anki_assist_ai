...
def single_word_profile(grammar_file, word, source_language, proficiency_level, generate_image=False, generate_tts=False, add_to_anki=False, lecture=1, ubung=1):
    word_profile = get_translation_and_example(word, source_language, proficiency_level, grammar_file, lecture, ubung)
    print(word_profile)
    if generate_image:
...
def word_list_profile(grammar_file, word_csv, source_language, proficiency_level, generate_image=False, generate_tts=False, add_to_anki=False):
    ...
    for index, row in word_csv.iterrows():
        ...
        single_word_profile(grammar_file, word, source_language, proficiency_level, generate_image, generate_tts, add_to_anki, lecture, ubung)
...
if __name__ == "__main__":
    ...
    # words = ["erlebnis", "dabei sein", "angenehm"]
    # for word in words:
    ...
    word_list_profile(grammar_file, df, source_language, proficiency_level, generate_image, generate_tts, add_to_anki)
